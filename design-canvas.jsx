  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      if (s && Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.scale)) {
        ...
        // No saved state — fit-to-content default so first paint shows the canvas
        const vp = vpRef.current;
        const w = worldRef.current;
        if (vp && w) {
          const vpW = vp.clientWidth, vpH = vp.clientHeight;
          // First artboard is ~1440 wide; fit width with padding
          const targetScale = Math.min(maxScale, Math.max(minScale, (vpW - 80) / 1500));
          tf.current = { x: 40, y: 40, scale: targetScale };
          apply();
        }
      }
    } catch {}
